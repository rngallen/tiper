...
export default function NewExternalReceiptPage() {
  ...
  const fieldErrors = validateReceiptHeader(form, "external");
  ...
  async function save() {
    ...
    try {
      const created = await api.post<{ id: string }>("/ic/receipts", {
        ...receiptHeaderPayload(form, "external"),
        details: [],
      });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="New external receipt"
        subtitle="Complete every header field, then add depot quantities on the document"
        actions={
          <Link href="/stock/receipts/external" className="text-sm font-medium text-slate-600 hover:text-slate-900">
            Back to list
          </Link>
        }
      />
      ...
      <Card className="space-y-5 p-6">
        ...
        <div className="flex justify-end gap-2 border-t border-slate-100 pt-4">
          <Button variant="secondary" onClick={() => router.push("/stock/receipts/external")}>
            Cancel
          </Button>
